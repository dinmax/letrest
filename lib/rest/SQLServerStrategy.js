...
var QUERY=function(service,action,entity,params,body,query,security) {
    ...
    var CONST_INSERT = "INSERT INTO %s (%s) OUTPUT inserted.%s VALUES(%s)  ";
    var CONST_UPDATE = "UPDATE %s SET %s OUTPUT inserted.%s WHERE %s ";
    ...
    this.parse=function() {
        ...
        switch (action) {
            ...
            case "insert":
                return LOG.show(MODULE,UTIL.format(CONST_INSERT,entity.table,processFields(),entity.pk, processValues()));
            case "update":
                return LOG.show(MODULE,UTIL.format(CONST_UPDATE,entity.table,processSets(),entity.pk, processWhere()));
            case "delete":
...
exports.insert=function(req) {
    ...
    DB.query(query.parse()).then(function(arr){
        var obj=req.body;
        obj[req.entity.pk]=arr[req.entity.pk];
        req.arr = obj;
        promise.resolve(req);
    }).catch(function(err){
        LOG.error(err);
