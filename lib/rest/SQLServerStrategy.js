...
var QUERY=function(service,action,entity,params,body,query,security) {
    var CONST_SELECT = "SELECT  *  FROM    ( SELECT  ROW_NUMBER() OVER (  %s) AS RowNum,  %s FROM %s %s WHERE %s %s) AS RowConstrainedResult %s";
    var CONST_INSERT = "INSERT INTO %s (%s) VALUES(%s)  "; //OUTPUT inserted.%s
    var CONST_UPDATE = "UPDATE %s SET %s WHERE %s "; //OUTPUT inserted.%s
    ...
    var getFKValue=function(fk) {
        ...
        if (fk.type=="url"||fk.type=='path') {
            console.log('getFKValue - URL',fk.value);
    ...
    var processType=function(value,filter) {
        ...
        try {
            switch ((type || 'string').toLocaleLowerCase()) {
                ...
                case "date":
                    var m = require("moment");
                    // save the double "" on the beginning and the end of value in some cases that caused problems
                    value = value.replace("\"\"","\"");
    ...
    var processLimit=function() {
        if (service=='all' && query.FROM && query.ROWS) {
            ...
            from=query.FROM;
            paginated=true;
            return "where  RowNum >=  "+query.FROM+" and RowNum <= "+ Math.min(CONFIG.max_rows,query.ROWS)+ ' order by RowNum ';
        }
        return 'where RowNum>=1 and RowNum <= '+CONFIG.max_rows +' order by RowNum ';
    ...
    this.parse=function() {
        ...
        //aca en el parse esta toda la posta para cambiar el query.
        ...
        switch (action) {
            case "select":
                ...
                return LOG.show(MODULE,UTIL.format(CONST_SELECT,processOrder(),defineFields(),entity.table,processJoin(),processWhere(),processGroup(),processLimit()));
            case "insert":
...
exports.insert=function(req) {
    ...
    DB.query(query.parse()).then(function(){
        var obj=req.body;
        req.arr = obj;
        var queryId = 'SELECT MAX('+req.entity.pk+') as '+req.entity.pk+' FROM '+req.entity.table;
        DB.query(queryId).then(function(arr) {
            // arr is now a resultset, so we need to fetch the 0 index!
            obj[req.entity.pk] = arr[0][req.entity.pk];
            promise.resolve(req);
        }).catch(function(err){
            LOG.error(err);
            promise.reject('Failed to execute query');
...
exports.update=function(req) {
    LOG.verbose(MODULE,"Update request");
    var promise = Q.defer();
