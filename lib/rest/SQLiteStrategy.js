...
var QUERY=function(service,action,entity,params,body,query,security) {
  ...
  var CONST_INSERT = "INSERT INTO %s (%s) VALUES(%s)";
  var CONST_UPDATE = "UPDATE %s SET %s WHERE %s";
  ...
  var getFKValue=function(fk) {
    ...
    if (fk.type=="url"||fk.type=='path') {
      ...
      console.log('getFKValue - URL or PATH',fk.value, value);
    } else {
      value= DOTTY.get(body,fk.value);
      console.log('getFKValue - DOTTY',fk.value, value);
    }
  ...
  var processLimit=function() {
    if (service=='all' && query.FROM && query.ROWS) {
      ...
      from=query.FROM;
      paginated=true;
      return "LIMIT "+query.FROM+", "+ Math.min(CONFIG.max_rows,query.ROWS);
    }
    //return ' OFFSET 0 LIMIT '+CONFIG.max_rows;
    //TODO a simple way to remove the limit from the queries
    return (entity.nolimit) ? '':'LIMIT 0,'+CONFIG.max_rows;
  };
  ...
  this.parse=function() {
    ...
    switch (action) {
      case "select":
        ...
        var totalRows = ",count(1) as __ROWSTOTAL";
      ...
      case "insert":
        return LOG.show(MODULE,UTIL.format(CONST_INSERT,entity.table,processFields(),processValues()));
      case "update":
        return LOG.show(MODULE,UTIL.format(CONST_UPDATE,entity.table,processSets(),processWhere()));
      case "delete":
...
exports.select=function(req) {
  ...
  DB.query(query.parse()).then(function(data){
    req.arr = query.enrichRows(data);
    promise.resolve(req);
  }).catch(function(err){
      LOG.error(err);
...
exports.insert=function(req) {
  ...
  DB.query(query.parse()).then(function(data){
    var obj=req.body;
    //TOFIX: what's the idea here?
    obj[req.entity.pk]=data.lastID;
    req.arr = obj;
    promise.resolve(req);
  }).catch(function(err){
      LOG.error(err);
...
exports.update=function(req) {
  ...
  DB.query((query.parse())).then(function(data){
    var obj=req.body;
    //TOFIX: what's the idea here?
    obj[req.entity.pk]=data.changes;
    req.arr = obj;
    promise.resolve(req);
  }).catch(function(err){
      LOG.error(err);
